...
const Input = forwardRef(({ className, label, error, onChange, min, type, ...props }, ref) => {
  const numberMin = type === 'number' ? min ?? 0 : min;
  const handleChange = (event) => {
    if (type === 'number' && event.target.value !== '') {
      const next = Number(event.target.value);
      const floor = Number(numberMin);
      if (Number.isFinite(next) && Number.isFinite(floor) && next < floor) {
        event.target.value = String(floor);
      }
    }
    onChange?.(event);
  };
